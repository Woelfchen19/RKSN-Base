pageextension 50009 "PInvoiceSubNVX" extends "Purch. Invoice Subform"
{
    layout
    {
                    ...
                    if ShortcutDimCode3 = '' then begin
                        if Rec.Type = Rec.Type::Item then begin
                            if not ItemNVX.Get(Rec."No.") then
                                ItemNVX.Init();
                            Item.Get("No.");
                            if (not ItemNVX."No bal line - Inv Value Zero") and Item."Inventory Value Zero" and (AllocationCodeVar = '') and (ShortcutDimCode1 <> '') then begin
        ...
        {
            field(PurchaseLineNVXVendUnitPriceNVX; PurchaseLineNVX."Vend. Unit Price")
            {
                ApplicationArea = All;
